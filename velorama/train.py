...
def train_model(config, checkpoint_dir = None):
	...
	if not os.path.exists(gc_path1) and not os.path.exists(gc_path2):
		...
		vmlp = VeloramaMLP(num_targets, num_regs,lag=lag, hidden=hidden, device=device, activation='relu')
		vmlp.to(device)
		...
		for it in range(max_iter):
			...
			if (it + 1) % check_every == 0:
				...
				nonsmooth = torch.sum(torch.stack([regularize(net, lam, penalty) for net in vmlp.networks]))
